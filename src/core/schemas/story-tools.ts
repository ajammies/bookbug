/**
 * Story Tools: Auto-generated tools from Story schema
 *
...
const createSetterTool = <T>(
  ...
  return tool({
    ...
    execute: async ({ value }): Promise<ToolResult> => {
      (state.story as Record<string, unknown>)[fieldName] = value;
...
const createAdderTool = (
  ...
  return tool({
    ...
    execute: async (input): Promise<ToolResult> => {
      ...
      const arr = (state.story as Record<string, unknown[]>)[fieldName];
      if (!arr) {
        (state.story as Record<string, unknown[]>)[fieldName] = [item];
      } else {
        arr.push(item);
      }
...
/**
 * Create all story tools bound to a state object
 *
...
export const createStoryTools = (state: StoryState): Record<string, any> => {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const tools: Record<string, any> = {};

  // Generate tools from schema shape
  for (const [fieldName, fieldSchema] of Object.entries(StorySchema.shape)) {
  ...
  tools.finishIntake = tool({
    ...
    execute: async ({ summary }): Promise<ToolResult> => {
      ...
      return { success: true, message: summary ?? 'Intake complete' };
    },
  });
  ...
  tools.updatePlotBeat = tool({
    ...
    execute: async ({ index, purpose, description }): Promise<ToolResult> => {
      const beats = state.story.plotBeats;
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export type StoryTools = Record<string, any>;
