...
export const PlotStructureSchema = z.object({
  storyArcSummary: z.string().min(1).describe('1-2 sentence emotional essence: protagonist, core conflict, transformation arc, and tone/style'),
...
export const PlotConversationResponseSchema = z.object({
  ...
  chips: z.array(z.string().min(1)).min(3).max(4).describe('Story-specific suggestions using character names/elements. Last chip must be approval like "Looks good - let\'s write it!"'),
